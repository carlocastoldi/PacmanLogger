...
class Logger(var logs: List[List[String]]) {
  ...
  val textGraphics = screen.newTextGraphics
	...
	//val filters = logs.map((l: List[String]) => List(l(2)))
	val filters = List("DOWNGRADED","INSTALLED","REINSTALLED","REMOVED","UPGRADED")
	val filterTable: OptionCursor = new OptionTable(List(" E","Filter By"), filters, List(true,true,true,true,true), mainTable, false, screen, textGraphics) with OptionCursor
